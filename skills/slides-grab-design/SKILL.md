...
5. If the slide needs a local video, store the video under `<slides-dir>/assets/`, reference it as `./assets/<file>`, and prefer a `poster="./assets/<file>"` thumbnail so PDF export uses a stable still image.
6. If the source video starts on YouTube or another supported page, use `slides-grab fetch-video --url <youtube-url> --slides-dir <path>` (or `yt-dlp` directly if needed) to download it into `<slides-dir>/assets/` before saving the slide HTML.
7. Run `slides-grab validate --slides-dir <path>` after generation or edits.
8. If validation fails, automatically fix the source slide HTML/CSS and re-run validation until it passes.
9. Run `slides-grab build-viewer --slides-dir <path>` only after validation passes.
10. Run the slide litmus check from `references/beautiful-slide-defaults.md` before presenting the deck for review.
11. Iterate on user feedback by editing only requested slide files, then re-run validation and rebuild the viewer.
12. Keep revising until user approves conversion stage.
